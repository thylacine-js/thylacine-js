...
const BUILD_DIR =
  process.env.APP_ENV === "production" ? "dist/production" : "dist/development";
...
export default async function () {
  ...
  const server = createProxyServer({ host, port, ssl, default_path: "/" });
  await server.listen(APP_PORT);
  console.log(
    `serving at ${process.env.APP_PROTOCOL}://${APP_HOST}${
