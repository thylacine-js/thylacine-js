...
${layouts
  .map(
    (layout) =>
      `import ${layout.class_name}, * as ${layout.class_name}_Star from '${appDir}/layouts/${layout.file_path}';`
  )
  .join("\n")}
${routes
  .map(
    (route) => `
import ${route.class_name}, * as ${route.class_name}_Star from '${appDir}/routes${route.file_path}';`
  )
  .join("\n")}
const routes = [
  ${[
